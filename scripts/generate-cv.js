...
async function generatePDF() {
  ...
  try {
    ...
    // Contact info
    doc.font('Helvetica').fontSize(10).text(`Phone: ${resumeData.basics.phone}`).moveDown(0.2);

    doc.text(`Email: ${resumeData.basics.email}`).moveDown(0.2);

    // LinkedIn (get from profiles)
    const linkedinProfile = resumeData.basics.profiles.find(
    ...
    doc.font('Helvetica-Bold').fontSize(10).text('• Programming Languages:').moveDown(0.3);
    ...
    if (programmingSkill) {
      programmingSkill.keywords.forEach(lang => {
        doc.font('Helvetica').fontSize(10).text(`- ${lang}`, { indent: 20 }).moveDown(0.2);
      });
    }

    // Frameworks/Platforms
    doc.moveDown(0.3);
    doc.font('Helvetica-Bold').fontSize(10).text('• Frameworks/ Platforms:').moveDown(0.3);

    const frameworksSkill = resumeData.skills.find(s => s.name === 'Frameworks/ Platforms');
    if (frameworksSkill) {
      frameworksSkill.keywords.forEach(framework => {
        doc.font('Helvetica').fontSize(10).text(`- ${framework}`, { indent: 20 }).moveDown(0.2);
      });
    }

    // Database Management Systems
    doc.moveDown(0.3);
    doc.font('Helvetica-Bold').fontSize(10).text('• Database Management Systems:').moveDown(0.3);

    const dbSkill = resumeData.skills.find(s => s.name === 'Database Management System');
    if (dbSkill) {
      dbSkill.keywords.forEach(db => {
        doc.font('Helvetica').fontSize(10).text(`- ${db}`, { indent: 20 }).moveDown(0.2);
      });
    }

    // Version Control
    doc.moveDown(0.3);
    doc.font('Helvetica-Bold').fontSize(10).text('• Version Control:').moveDown(0.3);

    const vcsSkill = resumeData.skills.find(s => s.name === 'Version Control');
    if (vcsSkill) {
      vcsSkill.keywords.forEach(vcs => {
        doc.font('Helvetica').fontSize(10).text(`- ${vcs}`, { indent: 20 }).moveDown(0.2);
      });
    }

    // Foreign Languages
    doc.moveDown(0.3);
    doc.font('Helvetica-Bold').fontSize(10).text('• Foreign Languages:').moveDown(0.3);
    ...
    doc.moveDown(0.3);
    doc.font('Helvetica-Bold').fontSize(10).text('• Others:').moveDown(0.3);

    doc
      .font('Helvetica')
      .fontSize(10)
      .text('- Good understanding of OOP methodologies, SOLID principles, design patterns', {
        indent: 20,
      })
      .moveDown(0.2);

    doc.text('- Experience with Agile and Scrum development process', { indent: 20 }).moveDown(0.2);
    ...
    sortedWork.forEach((job, index) => {
      ...
      doc.font('Helvetica-Bold').fontSize(12).text('Full Stack Developer', { continued: true });
      ...
      doc.text(dateRange, { align: 'right' }).moveDown(0.2);

      // Company name
      doc.font('Helvetica-Bold').fontSize(11).text(job.name).moveDown(0.2);

      // Project info if available
      if (job.projects && job.projects.length > 0) {
        // Add a single "Projects" heading
        doc.font('Helvetica-Bold').text('Projects:').moveDown(0.2);

        // Display each project
        job.projects.forEach((project, idx) => {
          // Project name in bold, indented
          doc.font('Helvetica-Bold').text(`- ${project.name}`, { indent: 20 }).moveDown(0.2);
      ...
      // Responsibilities
      doc.font('Helvetica-Bold').text('Responsibilities:').moveDown(0.3);
    ...
    if (resumeData.education && resumeData.education.length > 0) {
      ...
      doc.text(eduDateRange, { align: 'right' }).moveDown(0.3);

      doc.font('Helvetica').fontSize(10).text(edu.institution).moveDown(0.3);

      // GPA
      doc.text(`GPA: ${edu.score}`).moveDown(1);
    }
    ...
    if (resumeData.certificates && resumeData.certificates.length > 0) {
      ...
      resumeData.certificates.forEach(cert => {
        ...
        if (cert.name.toLowerCase().includes('microsoft')) {
          doc.font('Helvetica-Bold').fillColor('#0077B5');
    ...
    if (resumeData.projects && resumeData.projects.length > 0) {
      resumeData.projects.forEach(project => {
        doc.font('Helvetica-Bold').text(project.name).moveDown(0.3);

        doc.font('Helvetica').text(project.description, { align: 'justify' }).moveDown(0.3);

        // Add link if available
        if (project.url) {
          doc
            .text('Link: ', { continued: true })
            .fillColor('#0077B5')
